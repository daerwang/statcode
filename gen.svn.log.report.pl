...
sub setGV() {
	...
	$GV->{RevFullPathWithoutRepos} 	= $GV->{LogParsedInfo}->{OverallInfo}->{RevFullPathWithoutRepos};
	$GV->{RevFullPath} 			= $GV->{LogParsedInfo}->{OverallInfo}->{RevFullPath};
	$GV->{InPathPrefix4CoPaths} = substr($GV->{InPathPrefix}, 0, rindex($GV->{InPathPrefix}, '/') + 1);
	$GV->{FileStartPosInReport} = length($GV->{InPathPrefix4CoPaths});
	$GV->{LogFile}              = $assistor->get_repository_log_cmd_output_file($pms->{rev}, $GV->{ModuleInfo}->{log});
	$GV->{AllDfFile}            = $assistor->get_svn_log_mode_all_diff_output_file($pms->{rev}, $GV->{ModuleInfo});
	$GV->{NeedCoFirstVer} 		= $GV->{LogParsedInfo}->{NeedCoFirstVer};
	
	my $descRevs = $GV->{LogParsedInfo}->{RevsInfo}->{descRevs};
	$GV->{InitRev} = $descRevs->[$#{$descRevs}];
	$GV->{WithLoc} = defined($GV->{LogParsedInfo}->{OverallInfo}->{addLines});
	#$GV->{WithLoc} = 0;
	$GV->{ModuleBriefInfo} = '';
    ...
    my $overall = $GV->{LogParsedInfo}->{OverallInfo};
    $muData->{foc} = $overall->{filesCnt};
    if ($GV->{WithLoc}) {
	...
	$GV->{Error} = $GV->{LogParsedInfo}->{Error};
...
sub replacePart1PMS($){
	...
	my $loc = $ccvUtil->formatLoc4UI($GV->{LogParsedInfo}->{OverallInfo}, $GV->{WithLoc});
	my $foc = $GV->{LogParsedInfo}->{OverallInfo}->{filesCnt};
    ...
    my $svnLogFileUrl = $assistor->transOperateLocalPath2WebPath($GV->{LogFile});
    my $svnAllDfFileUrl = $GV->{WithLoc} ? $assistor->transOperateLocalPath2WebPath($GV->{AllDfFile}) : "javascript: void(0);";
	...
	$$part =~ s/#MODULE#/$GV->{ModuleInfo}->{module}/;
	$$part =~ s/#MODULE_URI#/$GV->{ModuleUrl}/;
	...
	$$part =~ s/#LOG_FILE#/$svnLogFileUrl/;
	$$part =~ s/#DF_FILE#/$svnAllDfFileUrl/;
...
sub replacePart2PMS($) {
	...
	my $showBin = 1;
	my $showDir = 1;
    my $DF = $assistor->getSvnMoudleAllRevsDFFile($pms, $MID);
    my $InitRevFileLocation = $assistor->getSvnMoudleInitRevFileLocation($pms, $MID);
    
    my $GV_IN_JS =<<GV_IN_JS;
{
	T_SNAP: "$T_SNAP",
	MID: "$MID",
	RevFullPath: "$GV->{RevFullPath}",
	RevFullPathWithoutRepos: "$GV->{RevFullPathWithoutRepos}",
	InPathPrefix: "$GV->{InPathPrefix}",
	InPathPrefix4CoPaths: "$GV->{InPathPrefix4CoPaths}",
	InitRev: "$GV->{InitRev}",
	WithLoc: "$GV->{WithLoc}",
	InitRevFileLocation: "$InitRevFileLocation",
	...
	ShowDir: "$showDir",
	DF: "$DF",	
	NeedCoFirstVer: "$GV->{NeedCoFirstVer}",
	ReportType: "SVN_LOG"
}
GV_IN_JS
...
sub writeUserViewItems() {
	my $usersInfo = $GV->{LogParsedInfo}->{UsersInfo};
...
sub writeFileViewItems() {
	my $filesInfo = $GV->{LogParsedInfo}->{PathsInfo};
	my $revsInfo = $GV->{LogParsedInfo}->{RevsInfo};
	my $pathsInfo = $GV->{LogParsedInfo}->{PathsInfo};
...
sub plusUserBriefInfo($$) {
    ...
    $GV->{ModuleBriefInfo} .= sprintf("%s, %d, %d, %d, %d\n",
                        $user, 
                        ($userInfo->{addLines} || 0) + ($userInfo->{delLines} || 0),
						($userInfo->{addLines} || 0),
						($userInfo->{delLines} || 0),                  
                        $userInfo->{filesCnt}
    );
...
sub generateBriefReportHeader() {
    $GV->{ModuleBriefInfo} = sprintf("Module: %s (%s)\n%s; Branch/Revisions: %s; DateScope: %s; By: %s,,\n",
                        $MID, 
                        $GV->{ModuleInfo}->{module},
                        $GV->{ModuleUrl},
                        $pms->{rev}, 
                        $pms->{date},
                        $pms->{wids}
    ...
    $GV->{ModuleBriefInfo} .= sprintf("Author, LOC, +, -, FOC\nAll users, %d, %d, %d, %d\n\n", 
                        ($overall->{addLines} || 0) + ($overall->{delLines} || 0),
						($overall->{addLines} || 0),
						($overall->{delLines} || 0),                  
                        $overall->{filesCnt}
    );    
